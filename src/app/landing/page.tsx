...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans selection:bg-yellow-500 selection:text-slate-950">
      {/* Top Banner Accent */}
      <div className="bg-gradient-to-r from-yellow-500 via-amber-500 to-yellow-600 px-4 py-2 text-center text-xs font-black text-slate-950 flex items-center justify-center gap-2">
        <Sparkles size={16} />
        <span>SITETRACKER CMD 2026 — Platform Digitalisasi Patroli K3 & Cacat Mutu Konstruksi Fisik</span>
        <span className="hidden sm:inline-block px-2 py-0.5 bg-slate-950 text-yellow-400 rounded-full text-[10px]">ISO 45001 & ISO 9001 Compliant</span>
      </div>

      {/* Hero Section */}
      <section className="relative overflow-hidden pt-16 pb-24 border-b border-slate-800/80 bg-[radial-gradient(ellipse_80%_80%_at_50%_-20%,rgba(234,179,8,0.15),rgba(255,255,255,0))]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          {/* Tagline Badge */}
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-yellow-500/10 border border-yellow-500/30 text-yellow-400 text-xs font-bold mb-8 backdrop-blur-md">
          ...
          <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link
              href="/"
              className="w-full sm:w-auto inline-flex items-center justify-center gap-3 px-8 py-4 bg-yellow-500 hover:bg-yellow-400 text-slate-950 font-black text-base rounded-2xl shadow-xl shadow-yellow-500/20 hover:scale-105 transition-all duration-200"
            >
      ...
      <section className="py-20 bg-slate-900/60 border-b border-slate-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <h2 className="text-xs font-extrabold text-yellow-500 tracking-widest uppercase">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {/* K3 Safety */}
            <div className="bg-slate-900/90 border border-slate-800 hover:border-yellow-500/50 p-6 rounded-3xl transition-all duration-300 hover:-translate-y-1">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
            ...
            <div className="relative bg-slate-900 p-8 rounded-3xl border border-slate-800">
              <div className="w-10 h-10 rounded-full bg-yellow-500 text-slate-950 font-black text-lg flex items-center justify-center mb-6">
                1
              </div>
      ...
      {/* CTA Footer Section */}
      <section className="py-20 bg-gradient-to-br from-yellow-500 to-amber-600 text-slate-950">
        <div className="max-w-5xl mx-auto px-4 text-center">
          <HardHat size={48} className="mx-auto mb-4" />
          <h2 className="text-3xl sm:text-5xl font-black tracking-tight">
            Siap Tingkatkan Kepatuhan K3 & Mutu Proyek Anda?
          </h2>
          <p className="mt-4 text-base sm:text-lg font-semibold text-slate-900 max-w-2xl mx-auto">
            Gunakan SiteTracker CMD hari ini untuk pemantauan patroli fisik yang transparan, akuntabel, dan real-time.
          </p>
          <div className="mt-8 flex justify-center gap-4 flex-wrap">
            <Link
              href="/"
              className="px-8 py-4 bg-slate-950 text-white font-black rounded-2xl shadow-2xl hover:scale-105 transition-transform"
            >
              Masuk ke Aplikasi Dashboard
            </Link>
            <Link
              href="/reports"
              className="px-8 py-4 bg-white/90 hover:bg-white text-slate-950 font-bold rounded-2xl shadow-xl transition-all"
            >
